...
if(!isset($jsonData->fullname) || !isset($jsonData->username) || !isset($jsonData->password) || !isset($jsonData->usertype)) {
  $response = new Response();
  $response->setHttpStatusCode(400);
  $response->setSuccess(false);
...
if(strlen($jsonData->fullname) < 1 || strlen($jsonData->fullname) > 255 || strlen($jsonData->username) < 1 || strlen($jsonData->username) > 255 || strlen($jsonData->password) < 1 || strlen($jsonData->password) > 100 ||  strlen($jsonData->usertype) < 1 || strlen($jsonData->usertype)>255 ) {
  $response = new Response();
  $response->setHttpStatusCode(400);
  $response->setSuccess(false);
...
$usertype = trim($jsonData->usertype);
...
try {
  ...
  if(($usertype !== "admin") && ($usertype !== "user")) {
    ...
    $response->setHttpStatusCode(409);
    $response->setSuccess(false);
    $response->addMessage("User type error");
    $response->send();
    exit;
  }
  ...
  $query = $writeDB->prepare('INSERT into tblusers (fullname, username, password, usertype) values (:fullname, :username, :password, :usertype)');
